  ...
  return (
    <section className="relative py-24 bg-transparent overflow-hidden border-t border-slate-200/50">
          ...
          <div>
            ...
            <ScrollReveal variant="fade-up" delay={0.1}>
              <h2 className="text-3xl md:text-4xl font-black text-slate-900 uppercase tracking-tight">
                OUR CORPORATE <br />
                <span className="text-brand-gradient">NETWORK</span>
              </h2>
            </ScrollReveal>
          </div>
      ...
      <div className="relative z-10">
        ...
        {/* CSS mask applied to the container instead of gradient overlays */}
          ...
          <div className="flex w-max animate-[marquee_40s_linear_infinite] hover:[animation-play-state:paused]">
            <div className="flex gap-6 md:gap-8 px-3">
              {[...supportersRow1, ...supportersRow1, ...supportersRow1].map((supporter, idx) => (
                <div 
                  key={`r1-${idx}`} 
                  className="flex items-center justify-center w-[220px] md:w-[280px] h-[100px] shrink-0 p-6 rounded-2xl bg-white/80 backdrop-blur-md border border-slate-200/50 hover:border-primary/30 hover:shadow-lg transition-all duration-300 group"
                >
                  <img 
                    src={supporter.logo} 
                    alt={supporter.name}
                    className="w-full h-full object-contain opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300"
                  />
                </div>
              ))}
            </div>

    ...
    >
      ...
      >
        ...
        <button
          type="button"
          onClick={onClose}
          className="absolute -top-4 -right-4 z-[400] w-11 h-11 rounded-full bg-[#140c06]/92 border border-[#E2E8F0] backdrop-blur-md flex items-center justify-center cursor-pointer transition-all duration-200 shadow-[0_4px_20px_rgba(0,0,0,0.6)] hover:scale-110 hover:bg-[#dc3c3c]/90"
          ...
          <div className="absolute inset-0 flex flex-col justify-between pointer-events-none"
            ...
            <div className="flex items-start p-5 pointer-events-none"
              ...
              <div>
                ...
                <p className="text-[#111827] font-black text-base uppercase tracking-wide mt-0.5">{reel.name}</p>
              </div>
            </div>
            {/* Ghost play */}
            ...
            <div className="p-5 pointer-events-auto" style={{ background: "linear-gradient(to top, rgba(0,0,0,0.8), transparent)" }}>
              ...
              <div className="flex items-center gap-3">
                <button onClick={toggle} className="w-9 h-9 flex items-center justify-center hover:scale-110 transition-transform">
                  {playing ? <Pause className="w-5 h-5 text-[#111827]" fill="white" /> : <Play className="w-5 h-5 text-[#111827] ml-0.5" fill="white" />}
                </button>
                <button onClick={() => { const v = vRef.current; if (v) { v.muted = !v.muted; setMuted(v.muted); } }} className="w-8 h-8 flex items-center justify-center hover:scale-110 transition-transform">
                  {muted ? <VolumeX className="w-4 h-4 text-[#111827]/60" /> : <Volume2 className="w-4 h-4 text-[#111827]/60" />}
                </button>
                <div className="ml-auto">
                  <button onClick={() => { const v = vRef.current; if (v?.requestFullscreen) v.requestFullscreen(); }} className="w-8 h-8 flex items-center justify-center hover:scale-110 transition-transform">
                    <Maximize2 className="w-4 h-4 text-[#111827]/60" />
                  </button>
                </div>
              </div>
            </div>
...
function PortraitCard({ reel, onPlay, className = "" }) {
  ...
  return (
    <div
      className={`relative overflow-hidden cursor-pointer group ${className} border border-[#E2E8F0]`}
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0" style={{ padding: "16px 20px" }}>
        <h3 className="font-black text-[#111827] uppercase tracking-tight" style={{ fontSize: 16, margin: 0, textShadow: "0 1px 4px rgba(0,0,0,0.8)" }}>{reel.name}</h3>
...
function LargeQuoteCard({ reel, className = "" }) {
  return (
    <div className={`relative overflow-hidden flex flex-col justify-between ${className} border border-[#E2E8F0]`}
...
function VideoCard({ reel, onPlay, className = "" }) {
  ...
  return (
    <div className={`relative overflow-hidden cursor-pointer ${className} border border-[#E2E8F0]`}
      style={{ borderRadius: 22, background: "#080808" }}
      onClick={() => onPlay(reel)}
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}
...
function SmallQuoteCard({ reel, className = "" }) {
  return (
    <div className={`flex flex-col justify-between overflow-hidden ${className} border border-[#E2E8F0]`}
...
function MiniCard({ reel, onPlay, className = "" }) {
  ...
  return (
    <div className={`flex gap-4 items-start overflow-hidden cursor-pointer ${className} border border-[#E2E8F0]`}
    ...
    <main style={{ background: "#020202", minHeight: "100vh", fontFamily: "var(--font-poppins, sans-serif)" }}>
      ...
      <div className="relative" style={{ zIndex: 1, padding: "0 clamp(16px, 3vw, 48px) 72px", maxWidth: 1480, margin: "0 auto" }}>
        <div className="bento-grid">
          ...
          {/* TAPTI — dark quote */}
          <div className="g-tapti bento-card relative overflow-hidden flex flex-col justify-between border border-[#E2E8F0]"
      ...
      <div className="relative" style={{ zIndex: 1, margin: "0 clamp(16px,3vw,48px) 80px", maxWidth: 1480, marginLeft: "auto", marginRight: "auto" }}>
        <div className="border border-[#E2E8F0]" style={{ borderRadius: 28, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(20px)", padding: "56px 64px", display: "flex", flexWrap: "wrap", alignItems: "center", justifyItems: "center", justifyContent: "space-between", gap: 32, position: "relative", overflow: "hidden" }}>
